...
[Project](https://r4ds.had.co.nz/workflow-projects.html), supporting our
...
is an extension of Futia et al.’s
([2024](https://doi.org/10.1186/s40317-024-00380-3)) analysis of
heuristic residency metrics in acoustic telemetry systems that includes
state-space models fitted using the
...
1.  **State-space model formulation.**
    ...
    - Formulate an acoustic observation model for the Lake Champlain
      passive acoustic telemetry system;
2.  **Simulation analyses.**
    - Simulate trajectories using the lake trout movement model;
    - Simulate observations arising from trajectories using the Lake
      Champlain acoustic observation model;
    - Compare the performance and sensitivity of particle and heuristic
      algorithms applied to simulated observations in terms of (i) how
      accurately and (ii) how precisely they reconstruct simulated
      patterns;
    - For the particle algorithms, evaluate whether sufficient
      information exists to estimate static parameters (in the movement
      and observation models) alongside individual locations;
3.  **Real-world analyses.**
    - Analyse real-world datasets using particle algorithms and
      heuristic methods;
...
1.  **`data-raw/`** contains raw datasets\*. This includes:
    ...
    - `mfutia/model_comparison/` contains datasets from
      [`mfutia/model_comparison/data`](https://github.com/mfutia/model_comparison/tree/main/data);
    - `model-move/blanchfield-et-al-2023/` contains movement datasets,
      from Blanchfield et
...
2.  **`data/`** contains processed datasets, algorithm inputs and
    outputs, generated by `*.R` scripts\*. This includes:

    - `input/` include input data for analyses, including:
      - `map.tif`: a map of the study area;
      - `real/`: additional inputs for real-world analyses
        (e.g. processed detection data);
    ...
    - `inst/` contains [RStudio](https://www.rstudio.com/)
      ...
      - `tree.rds` is a record of the project directory tree (as
        generated by `proj.templates::use_template_tree()` in
        `dev/01-dev.R`, see below);
...
6.  **`Julia/`** is the [Julia](https://julialang.org) project
    directory.
...
These directories are not currently available in the online version of
this repository\*.
...
3.  **Setup project**, following the instructions in `dev/02-clone.R`:
    ...
    - **Packages.** Work through `dev/02-clone.R` to use
      ...
      manually reinstalled via `02-clone.R`, if required.
...
Futia, M. et al. ([2024](https://doi.org/10.1186/s40317-024-00380-3)).
Modeling regional occupancy of fishes using acoustic telemetry: a model
comparison framework applied to lake trout. Animal Biotelemetry, 12:25.
<https://doi.org/10.1186/s40317-024-00380-3>

Lavender, E. and Futia, M. (in prep). State-space models improve
residency estimation in acoustic telemetry systems.

Lavender, E. and Futia, M. (in prep). State-space modelling of lake
trout movements in an acoustic telemetry system.
